...
int main(int argc, char *argv[]) {
  ...
  float mb_per_sec =
      ((float)bytes / (1024.0 * 1024.0)) / ((float)duration / 1000.0);

  float days_per_terrabyte =
      (1024.0 * 1024.0) / mb_per_sec / 60.0 / 60.0 / 24.0;
  std::cout << "Total: " << duration << " ms , MB/sec : " << mb_per_sec
            << ", days per Terrabyte: " << days_per_terrabyte << std::endl;
  std::cout << "Merging ... ";
  ...
  std::cout << "Total: " << duration
            << " ms, ms / 1000 tokens : " << time_per_1000_tokens << std::endl;
  ...
  std::cout << "Overall it took: " << seconds << " secs, or " << minutes
            << " minutes" << std::endl;
}
