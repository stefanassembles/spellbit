...
namespace spellbit {
  ...
  void Trie::insert(const std::vector<uint32_t> &token, uint32_t id) {
    ...
    for (uint32_t codepoint : token) {
      ...
      if (token_id >= alphabet_size) {
        std::cerr << "Error: Codepoint " << token_id << " exceeds alphabet size " << alphabet_size << std::endl;
        return;
      }
  ...
  std::optional<TokenMatch> Trie::find_longest_match(const std::vector<uint32_t> &text, size_t start) {
    uint32_t current = 0;
    ...
    for (size_t i = start; i < text.size(); ++i) {
      ...
      if (token >= alphabet_size || nodes[current].children[token] == INVALID_INDEX) break;
  ...
  bool Tokenizer::init(const std::string &vocab_filename) {
    ...
    if (magic_number != 0xBEEFBABE) {
      std::cerr << "Error: Invalid magic number in file: " << vocab_filename << std::endl;
      return false;
    }
    if (version != 1) {
      std::cerr << "Error: Unsupported version: " << static_cast<int>(version) << std::endl;
      return false;
    }
    ...
    for (uint32_t i = 0; i < vocab_size; ++i) {
      ...
      infile.read(reinterpret_cast<char *>(token.data()), length * sizeof(uint32_t));
      if (length == 1) {
  ...
  std::vector<uint32_t> Tokenizer::tokenize(const std::string &filename) const {
    ...
    while (file.read(buffer, sizeof(buffer)) || file.gcount() > 0) {
      ...
      for (size_t i = 0; i < chunkSize; ++i) {
        ...
        if (c_index == -1) {
          if (c_size <= 0) {
            std::cerr << "Malformed or unexpected UTF-8 byte: " << static_cast<int>(c) << "\n";
            c_index = -1;
            continue;
          }
          c_index = 0;
        ...
        if (c_index == max_size) {
          ...
          if ((encoded_char < 128) && (std::isspace(static_cast<unsigned char>(encoded_char)) || std::ispunct(static_cast<unsigned char>(encoded_char)))) {
            if (!current.empty()) {
              size_t pos = 0;
