...
private:
  ...
  static constexpr uint32_t INVALID_TOKEN = std::numeric_limits<uint32_t>::max() - 1;
  struct TrieNode {
    uint32_t token_id = INVALID_TOKEN;
    std::vector<uint32_t> children;

    explicit TrieNode(size_t alphabet_size): children(alphabet_size, INVALID_INDEX), token_id(INVALID_TOKEN) {}
  };
...
public:
  ...
  std::optional<TokenMatch> find_longest_match(const std::vector<uint32_t> &text, size_t start);
 };
